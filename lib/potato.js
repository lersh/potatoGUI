...
Potato.prototype.SymbolRequest = {
    ...
    Resolve: function (buff) {
        ...
        if (buff === null)
            return null;//解码失败返回空
        var msg = {};
        msg.flag = buff.slice(0, 1);
        msg.len = buff.slice(1, 3).readUInt16BE(0);
        msg.dst = {};
        msg.addr = buff.slice(3, len + 3);
        msg.dst.addr = addr.toString();
        msg.port = buff.slice(len + 3, len + 5).readUInt16BE(0);
        msg.timestamp = buff.slice(len + 5, len + 13).readIntBE(0, 8);
        msg.dst.port = port;
        return msg;
    },
}
...
Potato.prototype.SymbolPeply = {
    ...
    Resolve: function (buff) {
        buff = decipherGCM(buff, password);
        var msg = {};
        msg.flag = buff.slice(0, 1);
        msg.sig = buff.slice(1, 2).readInt8(0);
        msg.timestamp = buff.slice(2, 10).readIntBE(0, 8);
        return msg;
    }
